...
function eval_poly!(points, parms, n_pt, direction, radius, cntr)
  if direction
    start = 1
    stp = n_pt
    step = 1
...
function calc_th(start_par, end_par, cntr_poly; tol = 0.000001, n_part = 1047)
  n_iter = 0
  points_shared = Vector{PlPoint}(undef, n_part+1)
  points_shared[1] = PlPoint(compute_bernstein(cntr_poly[1,:], start_par), compute_bernstein(cntr_poly[2,:], start_par))
  points_shared[n_part+1] = PlPoint(compute_bernstein(cntr_poly[1,:], end_par), compute_bernstein(cntr_poly[2,:], end_par))
  params_shared = Vector{Float64}(undef, n_part+1)
  ...
  while true
    @sync for i in 1:2
       @spawn eval_poly!(points_shared, params_shared, bounds[i], directs[i], radius, cntr_poly)
    end
    n_iter+=1
